...
export type DailyCreditsSpent = {
  date: string;
  successful: number;
  failed: number;
};

export async function getDailyCreditsSpentAction(): Promise<DailyCreditsSpent[]> {
  ...
  const creditsMap = new Map<string, { successful: number; failed: number }>();

  // Initialize all days with zero values
  days.forEach((day) => {
    const dateKey = format(day, "MMM d");
    creditsMap.set(dateKey, { successful: 0, failed: 0 });
  });
  ...
  phases.forEach((phase: { completedAt: Date | null; creditsCost: number | null; status: string }) => {
    ...
    const current = creditsMap.get(dateKey);
  ...
  const result: DailyCreditsSpent[] = Array.from(creditsMap.entries()).map(
    ([date, credits]) => ({
      date,
      successful: credits.successful,
      failed: credits.failed,
    })
  );
